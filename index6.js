...
const scheduleJob = async (job) => {
  ...
  const isAvailableRule = {
    conditions: {
      all: [{
        fact: 'engineer',
        path: `availability.${job.day}.${timeSlotKey}`,
        operator: 'equal',
        value: true
      }]
    },
  ...
  if (bestEngineer) {
    ...
    engineers = engineers.map(e => {
      if (e.id === bestEngineer.id) {
        e.availability[job.day][timeSlotKey] = false;
      }
      return e;
    });
...
for (const slot of timeSlots) {
  const jobsInTimeSlot = jobs.filter(job => job.day === slot.day && job.timeSlot[0] === slot.time[0] && job.timeSlot[1] === slot.time[1]);
